...
现象：qwen3.8-max（推理模型）单次选择调用超过网关 60s 超时（504），整轮推荐 3-4 分钟。

优化措施与实测效果（整轮对话 3-4 分钟 → 25 秒）：
1. Top3 选择改为**单轮 JSON 调用**（去掉 tools 循环）：候选数据已含全部关键字段，
   Reflection 终检保证选择有效，省掉 1-3 次串行工具往返；
2. `LLM_EXTRA_BODY` 环境变量支持注入供应商参数（如 `{"enable_thinking": false}` 关闭推理模型思考）；
3. 追问意图分类**规则优先**：序数/关键词能明确意图（详情/更多/修改）时不调 LLM，
   只有未命中规则的模糊消息才走 LLM 分类；
4. 模型切换：交互式对话用快档模型（gpt-5.6-luna，选择任务实测 4.6s），
   qwen3.8-max 保留为可选配置（质量更高但会触发网关超时）。
